...
def main():
    ...
    s_enemy_group = pygame.sprite.Group()
    e_bullet_group = pygame.sprite.Group()
    door_group = pygame.sprite.Group()
    f_door_group = pygame.sprite.Group()

    #Where the door is drawn
    door.rect.x = 200
    door.rect.y = 100
    door_group.add(door)

    #Where the final door is drawn
    f_door.rect.x = 10
    f_door.rect.y = 10
    f_door_group.add(f_door)
    ...
    s_enemy = S_Enemy()
    all_sprites_group.add(s_enemy)
    s_enemy_group.add(s_enemy)
    ...
    while not done:
        ...
        #Getting enemy to shoot
        if pygame.time.get_ticks() % 3000 < 30:
            shoot = True 

        if shoot == True:
            for enemy in enemy_group:
                e_bullet = E_Bullet(enemy_x,enemy_y)
                e_bullet.rect.x = enemy.rect.x
                e_bullet.rect.y = enemy.rect.y
                shoot_time += 1
                AI_bullet_list.add(e_bullet)
                all_sprites_group.add(e_bullet)
                if shoot == True and shoot_time % 1 == 0:
                    shoot = False
            

        #Collision between player and enemy bullet
        for bullet in AI_bullet_list:
            hit = pygame.sprite.spritecollide(e_bullet, active_sprite_list, True)
            if hit:
                player.rect.x = 340
                lives -= 1
        ...
        for door in door_group:
            door_hit = pygame.sprite.spritecollide(player, door_group, True)
            if door_hit and current_level_no < len(level_list) -1:
                current_level_no += 1
                current_level = level_list[current_level_no]
                player.level = current_level
                player.rect.x = 350
        ...
        #Collision between final door and player
        for f_door in f_door_group:
            f_door_hit = pygame.sprite.spritecollide(player, f_door_group, True)
            if f_door_hit:
                finish_screen = True

            while finish_screen == True:
                    screen.fill(BLACK)
                    myfont = pygame.font.SysFont('freesansbold.ttf',50)
                    font = pygame.font.SysFont('freesansbold.ttf',50)
                    FinText = myfont.render('YOU WIN',5, WHITE)
                    screen.blit(FinText,(300,300))
                    pygame.display.flip()
        ...
        for enemy in enemy_group:
            player_hit = pygame.sprite.spritecollide(enemy, active_sprite_list, True)
            if player_hit:
                player.rect.x = 340
                player.rect.y = SCREEN_HEIGHT - player.rect.height
                lives -= 3

        #Collision between player and stronger enemy
        for s_enemy in s_enemy_group:
            player_hit = pygame.sprite.spritecollide(s_enemy, active_sprite_list, True)
            if player_hit:
                player.rect.x = 340
                player.rect.y = SCREEN_HEIGHT - player.rect.height
                lives -= 3
        ...
        #Collision between bullet and stronger enemy
        for bullet in bullet_group:
            hit_s_enemy = pygame.sprite.spritecollide(bullet, s_enemy_group, True)
...
##                hit_s_enemy
            ...
            #If bullet hits stronger enemy
            for bullet in hit_s_enemy:
                #Increase score
                score += 300
...
##            powerup_on = True
        ...
        if powerup_on == True:
            ...
            player.change_x = player.change_x + 0.1
        ...
        while game_over and not close:
            ...
            screen.blit(game_over_text, [340,300])
            pygame.display.flip()
            clock.tick(60)        
